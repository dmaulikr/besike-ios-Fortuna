...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(application: UIApplication, didFinishLaunchingWithOptions
    launchOptions: [NSObject: AnyObject]?) -> Bool {
      ...
      for string in loadJSON(positiveQuotesPath!) as [String]! {
        positiveQuotes.append(string)
      }
      
      for string in loadJSON(negativeQuotesPath!) as [String]! {
        negativeQuotes.append(string)
      }
  ...
  func loadJSON(path: String) -> AnyObject? {
    ...
    assert(err == nil, "Error parsing jsono: \(err)")
